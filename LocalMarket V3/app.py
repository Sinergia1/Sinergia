...
# ===== (COLOCADAS AL PRINCIPIO) =============
# ============================================

# Ruta específica para archivos estáticos normales
@app.route('/static/<path:filename>')
def serve_static_files(filename):
    """Sirve archivos estáticos normales como CSS, JS"""
    return send_from_directory('static', filename)

# Ruta específica para imágenes subidas (productos, perfiles, logos)
...
# Ruta para archivos HTML (sin interferir con las imágenes)
@app.route('/<path:filename>')
def serve_files(filename):
    """Sirve archivos HTML"""
    # Solo manejar archivos HTML
    if filename.endswith('.html'):
        try:
            return render_template(filename)
        except:
            return f"Archivo {filename} no encontrado", 404
    # Para cualquier otra cosa, intentar desde static
    try:
        return send_from_directory('static', filename)
    except:
        return f"Archivo {filename} no encontrado", 404
...
@app.route('/api/categorias/empresa', methods=['POST'])
def crear_categoria_empresa():
    ...
    try:
        cursor.execute("""
            ...
            WHERE empresa_id = %s AND categoria = %s AND activo = 1
        """, (empresa_id, nombre))
        
        existe = cursor.fetchone()[0] > 0
        
        if existe:
            return jsonify({'error': 'Ya existe una categoría con ese nombre en tus productos'}), 400
        
        return jsonify({
            'success': True, 
            'message': 'Categoría disponible para usar en tus productos',
            'categoria': nombre
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
@app.route('/api/categorias/empresa/actualizar', methods=['PUT'])
def actualizar_categoria_empresa():
    """Actualiza el nombre de una categoría en TODOS los productos de la empresa"""
    data = request.json
    ...
    nombre_antiguo = nombre_antiguo.strip()
    nombre_nuevo = nombre_nuevo.strip()
    
    if nombre_antiguo == nombre_nuevo:
        return jsonify({'error': 'Los nombres son iguales'}), 400
    ...
    try:
        cursor.execute("""
            ...
            SET categoria = %s 
            WHERE empresa_id = %s AND categoria = %s AND activo = 1
        """, (nombre_nuevo, empresa_id, nombre_antiguo))
        
        conn.commit()
        
        return jsonify({
            'success': True, 
            'message': f'Categoría actualizada en {cursor.rowcount} productos',
...
@app.route('/api/categorias/empresa/<path:nombre_categoria>', methods=['DELETE'])
def eliminar_categoria_empresa(nombre_categoria):
    """Elimina la categoría de todos los productos de la empresa"""
    ...
    try:
        cursor.execute("""
            ...
            SET categoria = '' 
            WHERE empresa_id = %s AND categoria = %s AND activo = 1
        """, (empresa_id, nombre_categoria))
        
        conn.commit()
        
        return jsonify({
            'success': True, 
            'message': f'Categoría eliminada de {cursor.rowcount} productos',
