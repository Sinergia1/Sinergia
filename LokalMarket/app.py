...
def get_db_connection():
    ...
    try:
        ...
        print(f"❌ Error conectando a PostgreSQL: {err}")
        return None
...
UPLOAD_FOLDER = '/tmp/uploads'  # En Render usamos /tmp
...
def buscar_empresa():
    ...
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute("SELECT * FROM empresa WHERE nombre ILIKE %s AND activo = true", (f'%{nombre}%',))
...
def get_empresas():
    ...
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute("SELECT * FROM empresa WHERE activo = true ORDER BY nombre")
...
def get_empresa_stats(empresa_id):
    ...
    try:
        # Total de productos
        cursor.execute("SELECT COUNT(*) as total FROM productos WHERE empresa_id = %s AND activo = true", (empresa_id,))
...
def registro():
    ...
    try:
        ...
        cursor.execute("""
            ...
            RETURNING id, nombre, correo, foto_url, direcciones, telefono, rol, empresa_id
...
def aprobar_solicitud(solicitud_id):
    ...
    try:
        ...
        try:
            msg = Message(
                subject="✅ ¡Tu solicitud en LokalMarket ha sido APROBADA!",
            ...
            <p>Accede a: <a href="https://localmarket-frontend.onrender.com/panel_empresa.html">Panel de Empresa</a></p>
            """
...
def get_stats():
    ...
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute("SELECT COUNT(*) as total FROM empresa WHERE activo = true")
...
if __name__ == '__main__':
    print("=" * 50)
    print("🚀 INICIANDO SERVIDOR LOKALMARKET (PostgreSQL)")
    print("=" * 50)
    print(f"📁 Upload folder: {UPLOAD_FOLDER}")
    print("=" * 50)
    print("👤 Usuarios:")
    print("   Admin: admin@lokalmarket.com / admin123")
    print("   Socio: socio@lokalmarket.com / socio123")
    print("🏢 Empresa: LokalMarket")
    print("=" * 50)
